...
SharedLink.prototype.canUse = function () {
  ...
  if (this.downloadLimit && this.usedDownloads >= this.downloadLimit)
    return false;
  return true;
};
...
/**
 * Reserve a download slot with one conditional update so simultaneous requests
 * cannot exceed a link's configured limit.
 */
SharedLink.consumeDownload = async function (sharedLinkId) {
  const now = new Date();
  const [affectedRows] = await SharedLink.update(
    {
      accessedCount: sequelize.literal("`accessedCount` + 1"),
      lastAccessedAt: now,
      usedDownloads: sequelize.literal("`usedDownloads` + 1"),
    },
    {
      where: {
        [Op.and]: [
          ...
          },
          {
            [Op.or]: [
              ...
              { downloadLimit: 0 },
              sequelize.where(
                sequelize.col("usedDownloads"),
                Op.lt,
                sequelize.col("downloadLimit"),
              ),
            ],
          },
        ],
...
SharedLink.prototype.consume = async function () {
  const consumed = await SharedLink.consumeDownload(this.id);
  if (!consumed) {
    return {
      success: false,
