...
async function checkPassword(sharedLink, file, providedPassword) {
    ...
    const isValid = await bcrypt.compare(providedPassword, sharedLink.passwordHash);
    if (!isValid) {
        return {
            status: 401,
...
async function streamSharedFile(res, file) {
    ...
    const encodedName = encodeURIComponent(file.displayFilename || file.originalFilename);
...
async function accessSharedLink(req, res) {
    try {
        ...
        const rejection = await checkPassword(sharedLink, file, req.query.password);
        if (rejection) {
        ...
        if (wantsDownload) {
            ...
            await sharedLink.recordDownload();
...
module.exports = { accessSharedLink };
